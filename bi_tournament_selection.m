function f = bi_tournament_selection(Group, pool_size, tour_size)
...
for i = 1 : pool_size
    ...
    for j = 1 : tour_size
        c_obj_rank(j) = Group(candidate(j),rank);
        c_obj_distance(j) = Group(candidate(j),distance);
    end
    ...
    if length(min_candidate) ~= 1
        ...
        f(i,:) = Group(candidate(min_candidate(max_candidate)),:);
    else
        % Add the selected individual to the mating pool
        f(i,:) = Group(candidate(min_candidate(1)),:);
    end
end
